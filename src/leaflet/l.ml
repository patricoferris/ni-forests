...
module TileLayer = struct
  ...
  let set_url ?no_redraw t url =
    Jv.call t "setUrl"
      [|
        Jv.of_string url; Jv.of_option ~none:Jv.undefined Jv.of_bool no_redraw;
      |]
    |> ignore
end
...
module Map = struct
  ...
  let add_layer t layer =
    let layer =
      match layer with
      | `Layer l -> Layer.to_jv l
      | `Tile l -> TileLayer.to_jv l
    in
...
module Control = struct
  ...
  let position_to_jstr = function 
    | TopLeft -> Jstr.v "top-left"
    | TopRight -> Jstr.v "top-right"
    | BottomLeft -> Jstr.v "bottom-left"
    | BottomRight -> Jstr.v "bottom-right"
  
  let create ?position ~on_add ~update () = 
    let args = match position with None -> Jv.undefined | Some s -> Jv.of_jstr (position_to_jstr s) in
